...
import { LLMErrorReporter } from '../services/llmErrorReporter';
    ...
    private handleCopyForAI = () => {
        ...
        try {
            ...
            navigator.clipboard.writeText(fullReport).then(() => {
                this.props.showToast('📋 Relatório completo copiado!', { type: 'success' });
            }).catch(() => {
                this.props.showToast('Falha ao copiar relatório', { type: 'error' });
            });
        } catch (error) {
            console.error('Error generating AI report:', error);
            this.props.showToast('Erro ao gerar relatório para IA', { type: 'error' });
        }
    };
                        ...
                        <div className="border-b border-red-500/20 bg-red-500/10 p-6 text-center">
                            ...
                            <p className="mt-2 text-zinc-400">
                                {this.state.error?.message?.includes('dynamically imported module')
                                    ? 'Parece que uma nova versão do app está disponível. Por favor, limpe o cache e atualize.'
