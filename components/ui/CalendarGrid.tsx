...
export const CalendarGrid: React.FC<CalendarGridProps> = ({ items }) => {
    const today = new Date();
    const daysInMonth = new Date(today.getFullYear(), today.getMonth() + 1, 0).getDate();
    const firstDayOfMonth = new Date(today.getFullYear(), today.getMonth(), 1).getDay(); // 0 = Dom, 1 = Seg...

    // Mapeia itens por dia do mês atual
    const itemsByDay = useMemo(() => {
        const map = new Map<number, ScheduledTransaction[]>();
        items.forEach(item => {
            const date = new Date(item.next_due_date);
            // Filtra apenas itens deste mês/ano
    ...
    return (
        <div className="grid grid-cols-7 gap-2 text-sm">
            {['D', 'S', 'T', 'Q', 'Q', 'S', 'S'].map((d, i) => (
                <div key={i} className="text-center text-gray-500 font-bold py-2">{d}</div>
            ))}
