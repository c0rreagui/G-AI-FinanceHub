...
DROP TABLE IF EXISTS public.family_invites CASCADE; -- Added missing drop
...
DROP TABLE IF EXISTS public.investments CASCADE; -- NEW
...
DROP TYPE IF EXISTS public.investment_type CASCADE; -- NEW
...
CREATE TYPE public.scheduled_transaction_frequency AS ENUM ('Diário', 'Semanal', 'Quinzenal', 'Mensal', 'Anual');
CREATE TYPE public.investment_type AS ENUM ('renda_fixa', 'acoes', 'fiis', 'cripto', 'exterior', 'outros'); -- NEW
...
CREATE TABLE public.investments (
  id uuid NOT NULL PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id uuid NOT NULL REFERENCES auth.users ON DELETE CASCADE,
  name text NOT NULL, -- Ex: "Petrobras", "Tesouro Selic"
  ticker text, -- Ex: "PETR4", "BTC"
  type public.investment_type NOT NULL,
  amount numeric(12, 2) NOT NULL CHECK (amount >= 0), -- Valor total investido
  quantity numeric(12, 4) NOT NULL DEFAULT 1, -- Quantidade de cotas
  current_price numeric(12, 2), -- Preço atual unitário (pode ser atualizado via API/Mock)
  purchase_date timestamptz NOT NULL DEFAULT now(),
  color text, -- Cor para gráficos
  logo_url text, -- URL do logo
  sector text, -- Ex: "Bancos", "Tecnologia", "Varejo"
  created_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE TABLE public.transactions (
  ...
  goal_contribution_id uuid REFERENCES public.goals ON DELETE SET NULL,
  debt_payment_id uuid REFERENCES public.debts ON DELETE SET NULL,
  investment_id uuid REFERENCES public.investments ON DELETE SET NULL, -- Link para aportes/dividendos
  created_at timestamptz NOT NULL DEFAULT now()
);
...
ALTER TABLE public.investments ENABLE ROW LEVEL SECURITY; -- NEW
...
-- User Profiles
CREATE POLICY "Usuários podem ver seu próprio perfil." ON public.user_profiles FOR SELECT USING ((select auth.uid()) = id);
CREATE POLICY "Usuários podem atualizar seu próprio perfil." ON public.user_profiles FOR UPDATE USING ((select auth.uid()) = id);

-- Categories
CREATE POLICY "Usuários podem gerenciar suas próprias categorias." ON public.categories FOR ALL USING ((select auth.uid()) = user_id);

-- Goals
CREATE POLICY "Usuários podem gerenciar suas próprias metas." ON public.goals FOR ALL USING ((select auth.uid()) = user_id);

-- Debts
CREATE POLICY "Usuários podem gerenciar suas próprias dívidas." ON public.debts FOR ALL USING ((select auth.uid()) = user_id);

-- Investments (NEW)
CREATE POLICY "Usuários podem gerenciar seus próprios investimentos." ON public.investments FOR ALL USING ((select auth.uid()) = user_id);

-- Transactions
CREATE POLICY "Users can view own and family transactions" ON public.transactions
  FOR SELECT USING (
    user_id = auth.uid() OR 
    user_id IN (SELECT user_id FROM public.user_families WHERE family_id = public.get_my_family_id())
  );
...
CREATE POLICY "Usuários podem gerenciar seus próprios agendamentos." ON public.scheduled_transactions FOR ALL USING ((select auth.uid()) = user_id);
...
-- Invites
CREATE POLICY "Users can view invites created by them" ON public.family_invites
  FOR SELECT USING (created_by = auth.uid());

CREATE POLICY "Users can create invites for their family" ON public.family_invites
...
-- Transaction Comments
CREATE POLICY "Users can view comments on visible transactions" ON public.transaction_comments
  FOR SELECT USING (
    EXISTS (
      ...
      AND (
        ...
        t.user_id IN (SELECT user_id FROM public.user_families WHERE family_id = public.get_my_family_id())
      )
    )
  );

CREATE POLICY "Users can comment on visible transactions" ON public.transaction_comments
  FOR INSERT WITH CHECK (
    EXISTS (
      SELECT 1 FROM public.transactions t
      WHERE t.id = transaction_comments.transaction_id
...
CREATE INDEX ON public.investments (user_id); -- NEW
...
BEGIN
  ...
  VALUES
    (new.id, 'Alimentação', 'Utensils', '#f59e0b'),
...
BEGIN
  IF (TG_OP = 'DELETE') THEN
    goal_id_to_update := OLD.goal_contribution_id;
  ELSE
    goal_id_to_update := NEW.goal_contribution_id;
  END IF;

  IF goal_id_to_update IS NULL THEN
    RETURN NULL;
  END IF;
  ...
  WHERE goal_contribution_id = goal_id_to_update AND type = 'despesa';
...
CREATE TRIGGER on_transaction_change_update_goal
  ...
  FOR EACH ROW
  EXECUTE PROCEDURE public.update_goal_current_amount();

-- Função para ATUALIZAR o valor pago de uma DÍVIDA.
CREATE OR REPLACE FUNCTION public.update_debt_paid_amount()
RETURNS TRIGGER AS $$
DECLARE
  debt_id_to_update uuid;
  new_paid_amount numeric(12, 2);
BEGIN
  IF (TG_OP = 'DELETE') THEN
    debt_id_to_update := OLD.debt_payment_id;
  ELSE
    debt_id_to_update := NEW.debt_payment_id;
  END IF;

  IF debt_id_to_update IS NULL THEN
    RETURN NULL;
  END IF;
  ...
  FROM public.transactions
  WHERE debt_payment_id = debt_id_to_update AND type = 'despesa';
...
CREATE TRIGGER on_transaction_change_update_debt
  AFTER INSERT OR UPDATE OR DELETE ON public.transactions
  FOR EACH ROW
  EXECUTE PROCEDURE public.update_debt_paid_amount();
...
ALTER PUBLICATION supabase_realtime ADD TABLE public.investments; -- NEW
